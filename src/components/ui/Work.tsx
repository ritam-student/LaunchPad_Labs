function Work(){
    return <div className=" bg-[#FFFFFF] rounded-[16px] px-2 pt-2 h-[316px] w-[300px] md:w-[381px]  ">
        <div className=" h-[258px] w-full bg-[#B9DDFB] rounded-[8px] "></div>
        <div className="w-full h-auto pt-1 pb-2 flex items-center justify-between">
            <p className="text-[#404040] text-[16px] font-semibold leading-[22px] md:leading-[31px] ">FitAI</p>
            <div className=" w-[22px] h-[22px] flex items-center justify-center overflow-hidden ">
                <img src="/clarity_arrow-line.png" alt="" className="h-full w-full object-contain"/>
            </div>
        </div>
    </div>
}

export default Work;